...
class MergeSchool extends Component {
    ...
    initializeBoard = () => {
        ...
        this.setState({board: boardset.board, qs_ranking: 32768, step: 0, gameover: false});
        // #########################
    }
    ...
    moveGrid = (direction) => {
        if (!this.state.gameover) {
            ...
            // # 8 Implement yourself
                ...
                let nextBoard = this.moveRight(this.rotateCounterClockwise(this.state.board));
                nextBoard = this.moveDown(this.rotateClockwise(nextBoard.board));
                this.checkAndUpdateAfterMove(nextBoard);
            }
            else if (direction === 'left'){
                let nextBoard = this.moveRight(this.rotateClockwise(this.rotateClockwise(this.state.board)));
                nextBoard = this.moveLeft(this.rotateClockwise(this.rotateClockwise(nextBoard.board)));
                this.checkAndUpdateAfterMove(nextBoard);
            }
            else if (direction === 'up'){
                let nextBoard = this.moveRight(this.rotateClockwise(this.state.board));
                nextBoard = this.moveUp(this.rotateCounterClockwise(nextBoard.board));
                this.checkAndUpdateAfterMove(nextBoard);
            }
            // #########################
        }
    ...
    checkAndUpdateAfterMove = (nextBoard) => {
        if (this.justifyMove(this.state.board, nextBoard.board)) {
            ...
            compareTwoBoard(this.state.board,nextBoard.board);
            // #########################
            ...
            const checkNew =(m1,m2) =>{
                if (maxnum(m2)>maxnum(m1)){
                    qsRankNow -=1;
                }
            }
            checkNew(this.state.board,nextBoard.board);
            // #########################
            ...
            if (this.state.qs_ranking < this.state.best_qs_ranking){
                this.setState({best_qs_ranking: this.state.qs_ranking});
            }
            // #########################
    ...
    checkGameover = (board) => {
        ...
        let newBoard= this.moveRight(board);
        if(!this.justifyMove(board, newBoard.board)){
            board = this.rotateCounterClockwise(board);
            newBoard = this.moveRight(this.rotateCounterClockwise(newBoard.board));
            if(!this.justifyMove(board, newBoard.board)){
                board = this.rotateCounterClockwise(board);
                newBoard = this.moveRight(this.rotateCounterClockwise(newBoard.board));
                if(!this.justifyMove(board, newBoard.board)){
    ...
    render() {
        return (
            <>      
                <Header 
                qs_rank={this.state.qs_ranking} 
                step={this.state.step} 
                best={this.state.best_qs_ranking}
                newGame={this.initializeBoard}/>
                <Board2048 
                className="wrapper" 
                board={this.state.board} 
                gameover={this.state.gameover} 
                tryAgain={this.initializeBoard}/>
